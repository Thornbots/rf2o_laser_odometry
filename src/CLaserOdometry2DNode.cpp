...
int main(int argc, char** argv)
{
  ...
  executor.add_node(myLaserOdomNode);

  // Loop. spin_all drains every queued message, where spin_some takes one
  // per subscription: 100 Hz odom_prior_topic would back up at 20 Hz.
  while (rclcpp::ok()){
      executor.spin_all(std::chrono::milliseconds(20));
